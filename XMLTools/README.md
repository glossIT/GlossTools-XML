...
This toolbox provides scripts and jupyter notebooks for checking and manipulating XML data, applying XSLT
transformations, etc. In this toolbox, there are currently two projects:

1. **METS Output Sanity Checks:** Perform sanity checks on an eScriptorium METS output regarding GlossIT specifications
   such as: Regions must be unique (e.g., no two regions marked as upper marginal zone), 
2. **Connecting Glosses (TBD):**
